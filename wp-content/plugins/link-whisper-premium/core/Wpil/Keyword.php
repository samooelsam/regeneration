...
class Wpil_Keyword
{
    ...
    public static function checkPosts($keyword, $return = false)
    {
        ...
        if (empty($posts) && !$at_limit) {
            ...
            $results = $wpdb->get_results("SELECT ID FROM {$wpdb->posts} WHERE {$keyword_search} $statuses_query {$user_role_ignore} $where $when
                                                    UNION
                                                    SELECT p.ID FROM {$wpdb->posts} p INNER JOIN {$wpdb->postmeta} m ON p.ID = m.post_id WHERE m.meta_key IN ('_themify_builder_settings_json', 'ct_builder_shortcodes', 'mfn-page-items-seo') AND m.meta_value LIKE {$case_sensitive} '%{$keyword->keyword}%' $statuses_query_p {$user_role_ignore_p}
                                                     $where");
    ...
    public static function addKeywordsToPost($post)
    {
        ...
        foreach ($keywords as $key => $keyword) {
            ...
            if (
                ...
                empty(preg_match('/(?<![a-zA-Z][\'\-_]|[a-zA-Z]|="|=\\\"|\/)'.preg_quote($keyword->keyword, '/').'(?![a-zA-Z]|[\'\-_][a-zA-Z]|<\/a>|<\\\\\/a>)/', $lowercased_content, $matches)) || empty($matches)) // OR the keyword only shows up inside of existing lnks
            {
                unset($keywords[$key]);
                continue;
            }
     ...
     **/
    public static function remove_unpublished_post_links($keyword){
        global $wpdb;
...
//            self::cleanupDeletedPostIds($keyword); // todo make if needed to clear links pointing to deleted posts
            ...
            if(!$removed_all){
                ...
                self::removeKeywordsFromIgnoredClasses($keyword);
            }
/*
...
*/
     ...
     **/
    public static function removeKeywordsFromIgnoredClasses($keyword){
        global $wpdb;
        $table = $wpdb->prefix . 'wpil_keyword_links';
        $memory_break_point = Wpil_Report::get_mem_break_point();
