...
//                                'wpil_sync_report3'                         => 1,
    ...
    public static function insert_links_into_link_table($post){
        ...
        $insert_query = "INSERT INTO {$links_table} (post_id, target_id, target_type, clean_url, raw_url, host, anchor, internal, has_links, post_type, location, broken_link_scanned, link_whisper_created, is_autolink) VALUES ";
        $links_data = array();
        $place_holders = array();
        foreach($links as $link){
            array_push (
            ...
            $place_holders [] = "('%d', '%d', '%s', '%s', '%s', '%s', '%s', '%d', '%d', '%s', '%s', '%d', '%d', '%d')";
        }
